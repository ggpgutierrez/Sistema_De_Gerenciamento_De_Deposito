    ...
    class Produto
    {
        ...
        public bool Atualizar(Produto produtos)
        {
            ...
            using (var con = connection.Conectar())
            {
                ...
                con.Open();
                MessageBox.Show(produtos.nomeProduto + produtos.ValorUnit + produtos.TipoProduto + produtos.UnidadeMedida + produtos.DescricaoProduto + produtos.Qtd + produtos.idProduto);
        ...
        public string CaixaDia()
        {
            ...
            {
                con.Open();
                MySqlCommand commd = new MySqlCommand("SELECT CASE WHEN FORMAT(SUM(p.Total), 2)  IS NULL THEN 'NULO' WHEN FORMAT(SUM(p.Total),2) > 0 THEN FORMAT(SUM(p.Total),2)  END FROM tb_pedido AS p WHERE CONVERT(p.Data_Pedido, char(10)) = Convert(CURRENT_DATE(), char(10)) AND p.Status = 'Faturado'", con);
                MySqlDataReader reader = commd.ExecuteReader();          
                    while (reader.Read())
                    {
